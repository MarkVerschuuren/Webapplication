...
def queries():
    ...
    cursor.execute("SELECT gene. geneName, stressaspect . aspect, organism . species, organism . genus, article . pubmedID "
                   "FROM gene, stressaspect, organism, article") # query voor de database
    ...
    #
    # print(termlist)
    return render_template("home.html", data=[{'name': 'Organism'}, {'name': 'Gen'}, {'name': 'Stressrespons'}], organism = termlist[0] + " " + termlist[1], amount = termlist[2])
